;define([
	...
	'storage',
	'views/questions',
	'views/classCourse-courseDetail',
	'views/classCourseTree',
	'views/activationCourse',
	'views/d3-stack',
	'layer'
	],function($, _, Backbone, storage, questions, courseDetail, classCourseTree, activationCourse, d3Stack, layer){
		'use strict';
		var Studycenter = Backbone.View.extend({
			el : 'body #right',
			template : _.template($('#template-class-course-courseStudy').html()),
			events : {
			...
			render : function(courseId){
				...
				CAICUI.render.$this.classRender(function(data){
					data.classCourseList.studyIn.forEach(function(classElement, classIndex){
						classElement.classCourse.forEach(function(courseElement, courseIndex){
							if(courseElement.courseId == CAICUI.render.courseId){
								var templateHtml = $('#template-class-course-nav').html();
								var addHtml = CAICUI.iGlobal.getTemplate(templateHtml,{
									"classId" : classElement.classId,
									"courseId" : courseElement.courseId,
									"classCourseNav" : classElement.classCourse
								});
								$('body .teachingPlan-course-nav').html(addHtml);
							}
						})
						// console.log(element.classId + ';' + CAICUI.render.classId)
						// if(element.classId == CAICUI.render.classId){
						// 	console.log(element.classCourse)
						// 	var templateHtml = $('#template-class-course-nav').html();
						// 	var addHtml = CAICUI.iGlobal.getTemplate(templateHtml,{
						// 		"classId" : CAICUI.render.classId,
						// 		"courseId" : CAICUI.render.courseId,
						// 		"classCourseNav" : element.classCourse
						// 	});
						// 	$('body .teachingPlan-course-nav').html(addHtml);
						// }
					})
				});
				CAICUI.render.$this.classCourseRender(function(data){
					$('body .course-change').attr('title',data.courseInfo.courseName);
					$('body .course-name').html(data.courseInfo.courseName);

					$('body .lastLearn').attr('href', 'javascript:;');
					$('body .continue-learning').attr('href', 'javascript:;');
					$('body .lastLearn').html(data.lastLearn.title);

					$('body .course-info-examTime').html(data.courseStatus.examinationDate);

					$('body .course-info-dueTime').html(data.courseStatus.courseExpirationTime);
					$('body .teachingPlan-span').each(function(index){
						$(this).text(data.courseStatus.countDownArray[index]);
					})

					$('body .task-number').html(data.tasksSummary.completed+'/'+data.tasksSummary.total);
					$('body .task-current-progress').css('width',data.studyInfo.studyProgressTotal);

					// CAICUI.render.courseDetailRender = new courseDetail();
					// $('body #scroller-courseStudy-list').html(CAICUI.render.courseDetailRender.render(data.planInfo).el);

					CAICUI.render.classCourseTreeRender = new classCourseTree();
					$('body #scroller-courseStudy-list').html(CAICUI.render.classCourseTreeRender.render(data.planInfo).el);

				});
				
			},
			classRender : function(callback){
				CAICUI.Request.ajax({
			...
			classCourseRender : function(callback){
				CAICUI.Request.ajax({
			...
			refreshHtml : function(){
				...
				if(CAICUI.isNoneRender){
					...
					CAICUI.render.$this.courseDetailAjax(function(){
						...
						var templateHtml = $('#template-class-courseStudy-availability').html();
						var addHtml = CAICUI.iGlobal.getTemplate(templateHtml,{
							"data" : {
								...
								CAICUI.render.$this.memberGetplanAjax(function(weekListData){
									...
									if(courseModel && courseModel.length){
										...
										$('body .courseIndex-baseInfo-btn').on('click',function(){
											var templateHtml = $('#template-class-course-baseInfo').html();
											var addHtml = CAICUI.iGlobal.getTemplate(templateHtml);
									...
									CAICUI.render.$this.courseDetailRender();
									if(CAICUI.render.courseDetailData && (CAICUI.render.courseDetailData.weekTotalBeoverdue >= CAICUI.render.weekMaxBeoverdue)){
			...
			courseDetailRender : function(){
				...
				if(CAICUI.render.courseDetailData){
					...
					// var that = $('body .task-current-progress');
					...
					// 	},1000);
					// }
					if(CAICUI.render.courseDetailData.coursePercentage){
						$('body .task-current-progress').animate({
							width: '40%'
						},1000);
					}
					$('body .task-number').html(CAICUI.render.courseDetailData.courseTaskTotalCompleted+'/'+CAICUI.render.courseDetailData.courseTaskTotal)
				}
				// if(CAICUI.render.courseDetailData.isPlan){
				// 	$('body #d3-stack').empty();
			...
			courseChange : function(e){
				...
				if(that.hasClass('active')){
					...
					if(CAICUI.render.learningcourseData && CAICUI.render.learningcourseData.length){
						var templateHtml = $('#template-class-courseStudy-courseList').html();
						var addHtml = CAICUI.iGlobal.getTemplate(templateHtml,{
		  ...
		  addAnimate : function(callback){
					...
					courseVersionList.removeClass('active');
						...
						if(CAICUI.render.versionId){
							if(CAICUI.render.courseVersion && CAICUI.render.courseVersion.length){
								var templateHtml = $('#template-class-courseStudy-courseVersionList').html();
								var addHtml = CAICUI.iGlobal.getTemplate(templateHtml,{
			...
			applyEditorLearningPlan : function(){
				var templateHtml = $('#template-class-apply-editor-learningPlan').html();
				var addHtml = CAICUI.iGlobal.getTemplate(templateHtml);
