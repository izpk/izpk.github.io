;define([
	...
	'layer'
		...
		var Studycenter = Backbone.View.extend({
			el : 'body',
			template : _.template($('#template-questions-realImitate').html()),
			events : {
				...
				'click .js-close-sidebar' : 'cloceSidebar',
			...
			init : function(){
				this.getExamenInfoAjax(function(){
					$('body .questions-title').html(CAICUI.render.examenInfo.title)
				});
				this.exerciseFilenameAjax(function(){

					var templateHtml = $('#template-questions-realImitate-cards').html();
					...
					feedback.on('click','.pop-button-confirm',function(){
						...
						if(!feedbackContent){
							...
							var jsonName = {
								...
								courseName : "真题模考",
								chapterName : "试题纠错",
								taskName : CAICUI.render.examenInfo.title,
							...
							if(CAICUI.render.questionsIndex == "0"){
								...
								sort = +CAICUI.render.questionsIndex+1;
							}
							addDom += 'data-type="realImitate" data-sort="'+sort+'" data-exercise-id="'+ CAICUI.render.exerciseId +'" data-title="'+ CAICUI.render.examenInfo.title+'" ';
			...
			createGuide : function(){
				var templateHtml = $('#template-questions-realImitate-guide').html();
				var addHtml = CAICUI.iGlobal.getTemplate(templateHtml);
				$('body .questions').append(addHtml);
			},
			...
			getExamenInfoAjax: function(callback){
				CAICUI.Request.ajax({
					'server' : 'getExamenInfo',
					'data' : {
			...
			addDOMNidExerciseDetail:function(data){
				...
				var templateHtml = $('#template-questions-realImitate-content').html();
				var addHtml = CAICUI.iGlobal.getTemplate(templateHtml,{
					'item' : exerciseData
				});
					...
					if(CAICUI.render.isMyChecked){
						...
						if(CAICUI.render.exerciseIdOld && CAICUI.render.exerciseIdNew){
							var requestData = {
								...
								'examenTotalNum' : CAICUI.render.exerciseBaseInfo.length,
								'examenName' : CAICUI.render.examenInfo.title,
			...
			questions : function(type,context,answerResolution){
				...
				CAICUI.render.isDone = $('body #questions-card-'+CAICUI.render.exerciseId).attr('data-isdone');
				console.log(CAICUI.render.isDone)
				console.log(CAICUI.render.isDoneNew)
		  ...
		  questionsResolution: function(type, context, answerResolution){
		  	var context = CAICUI.render.pushContext;
		  ...
		  removeAnimate : function(callback){
			...
			exit : function(){
				...
				this.exitMsg(function(){
					...
					// var link = '#questionBankList/'+CAICUI.render.examType+'/'+CAICUI.render.examId;
					// console.log(link);
					// window.location.hash = '#questionBankList/'+CAICUI.render.examType+'/'+CAICUI.render.examId;
			...
			questionsLoaderContent : function(){
				var templateHtml = $('#template-questions-realImitate-loader-content').html();
				var addHtml = CAICUI.iGlobal.getTemplate(templateHtml);
