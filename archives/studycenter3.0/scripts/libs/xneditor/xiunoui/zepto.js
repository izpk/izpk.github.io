...
;(function($, undefined){
	...
	$.fn.animate = function(properties, duration, ease, callback, delay){
		...
		// 如果为 scrollTop 则调用单独的动画函数, powered by axiuno@gmail.com
		
		if(properties.scrollTop !== undefined) {
			if(!duration) duration = 2;
			var node = this[0];
			var startpos = node.scrollTop;
			if(properties.scrollTop[0] == '+') {
				var endpos = Math.min(node.scrollHeight - this.height(), startpos + parseInt(properties.scrollTop));		
			} else if(properties.scrollTop[0] == '-') {
				var endpos = Math.max(-(node.scrollHeight - this.height()), startpos + parseInt(properties.scrollTop));
			} else {
				var endpos = parseInt(properties.scrollTop);
			}
			var delta = endpos - startpos; // 差值
			var starttime = Date.now();
			function scroll() {
				var fraction = Math.min(1, (Date.now() - starttime) / (duration * 1000)); // 时间进度百分比
				fraction = fraction * fraction * 3;
				node.scrollTop = delta * fraction + startpos;
				//console.log("fraction: %s, node.scrollTop: %s, delta: %s, startpos: %s", fraction, node.scrollTop, delta, startpos);
				if(fraction < 1) {
...
// 克隆节点
;(function($){
	$.fn.clone = function(cloneEvent) {
		return this.map(function(){
			var clones = this.cloneNode(true);
			var jclones = $(clones);
			// 克隆事件：遍历 this 子节点，挨个克隆
			if(cloneEvent) {
				var handlers = $.event.findHandlers(this, '');
				jclones.children().add(jclones).each(function() {
					var element = this;
					for(var i=0; i<handlers.length; i++) {
						var handler = handlers[i];
						$.event.add(element, handler.e, handler.proxy, null, handler.sel, handler.del, false);
					};
				});
			}
			return clones;
	...
	$.fn.delay = function(ms) {
		var _this = this;
		console.log('before ms:'+ms);
		setTimeout(function() {
			console.log('after ms:'+ms);
			_this.resolveWith(_this);
		}, ms);
		return this;
	}
	
	$.fn.xwidth = function(width) {
		var _this = this;
		this.then(function() {
...
// fix zepto.js: 在 box-sizing: border-box 模型下，结果与 jquery 保持一致 by axiuno
// jquery, zepto 计算 width() 不一致，在 box model 为内嵌时，jquery 减掉了 padding 的值。这时直接获取 css('width') 可以达到一致
$.fn.outerWidth = $.fn.width;
$.fn.outerHeight = $.fn.height;
$.fn.width = function(w) {
	if(this[0] === window) return this.outerWidth();
	if(this[0].style && this[0].style.paddingLeft) {
		var padding = intval(this[0].style.paddingLeft) + intval(this[0].style.paddingRight) + intval(this[0].style.borderLeftWidth) + intval(this[0].style.borderRightWidth);
	} else {
		var padding = 0;
	}
	if(w === undefined) {
		return this.outerWidth() - padding;
	} else {
		this.outerWidth(typeof w == 'number' ? w + padding : w);
		return this;
	}
}
$.fn.height = function(h) {
	if(this[0] === window) return this.outerHeight();
	if(this[0].style && this[0].style.paddingLeft) {
		var padding = intval(this[0].style.paddingTop) + intval(this[0].style.paddingBottom) + intval(this[0].style.borderTopWidth) + intval(this[0].style.borderBottomWidth);
	} else {
		var padding = 0;
	}
	if(h === undefined) {
		return this.outerHeight() - padding;
	} else {
		this.outerHeight(typeof h == 'number' ? h + padding : h);
		return this;
	}
}